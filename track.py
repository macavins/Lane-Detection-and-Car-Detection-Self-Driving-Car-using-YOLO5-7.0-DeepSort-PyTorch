...
def detect(opt):
    ...
    #############################################
    init=True
    mtx, dist = lane_detection.distortion_factors()
    #############################################
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if seen==1:
                ...
                if vid_cap:  # video
                    ...
                    fps, width, height = 30, im0.shape[1], im0.shape[0]
            ...
            # Lane finding 2021/12/28 #
            ###################################################################################################
            res_out, angle, warp_color, draw_poly_img = lane_detection.lane_finding_pipeline(im0, init, mtx, dist)
            ...
            annotator = Annotator(res_out, line_width=2, pil=not ascii)
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if cls == 2 or cls == 5 or cls == 7:
                            ...
                            apx_distance = round((((height-bboxes[3]))*ratio)*4.5,1)
                            mid_xy =[mid_x,mid_y]
